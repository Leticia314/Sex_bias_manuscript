...
Looking at expression of male and female-biased genes on single cell dataset (with male and female samples) to understand if sex-biased gene expression comes from specific cell types. In this specific example we are re-analysing the data from Ransick et al. (https://pubmed.ncbi.nlm.nih.gov/31689386/).
...
kidney_SB <- subset(SB_genes, Kidney==TRUE)
...
print("The following FB genes are not in the single-cell dataset: ")
table(kidney_MB$external_gene_name %in% rownames(SB_data.combined.sct))
...
ggplot(SB_data.combined.sct_vln@meta.data, aes(x=cellTypes, y=MB_scores)) + 
  geom_violin(aes(fill = sex))  + stat_pvalue_manual(stat.test, label = "p.adj.signif", size = 3) + theme(text = element_text(size = 15))  + theme(axis.text.x = element_text(angle = 45, hjust=1)) + ggtitle("Male-biased genes") + ylab("MB scores") + theme_classic2()+ scale_fill_manual(values=c("#895AA5","#F26622"))
```
...
print("The following FB genes are not in the single-cell dataset: ")
table(kidney_FB$external_gene_name %in% rownames(SB_data.combined.sct))
...
ggplot(SB_data.combined.sct_vln@meta.data, aes(x=cellTypes, y=FB_scores))+ geom_violin(aes(fill = sex))   + stat_pvalue_manual(stat.test, label = "p.adj.signif", size = 3) + theme(text = element_text(size = 15))  + theme(axis.text.x = element_text(angle = 45, hjust=1)) + ggtitle("Female-biased genes") + ylab("FB scores") 
```
