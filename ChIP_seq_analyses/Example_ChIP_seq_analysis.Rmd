...
getting_targets <- function(peak_data, dist=10000){
  ...
  TF_targets$near <- ifelse(TF_targets$distanceToTSS<dist, T, F)
...
Mouse_rpkm <- read.csv("../RPKMs/110122_Mouse_rpkm_all_genes.txt", sep="")
...
```{r, warning=F, message=F}
# Cux targets
Cux2_peaks1 <- read_delim("~/R_analyses/160621_ChIPseq_newcalls/Mouse_Liver/EXP031263.liver.CUX2.MA0755.1.damo.bed", 
    "\t", escape_double = FALSE, col_names = FALSE, 
    trim_ws = TRUE)
Cux2_peaks2 <- read_delim("~/R_analyses/160621_ChIPseq_newcalls/Mouse_Liver/EXP031264.liver.CUX2.MA0755.1.damo.bed", 
    "\t", escape_double = FALSE, col_names = FALSE, 
    trim_ws = TRUE)
...
```{r}
# Filtering background genes (stage=P63, max(rpkm) > 1)
